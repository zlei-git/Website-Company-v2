...
@section('title', 'Edit Address - Danone Store Indonesia')
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="border-b border-[#E5E0D8] pb-4 mb-8">
        ...
        <h1 class="font-serif text-3xl text-[#1C1C1C]">Edit Address</h1>
    </div>
    ...
    <div class="bg-white border border-[#E5E0D8] rounded p-6 sm:p-8">
        <form action="{{ route('customer.addresses.update', $address->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-[#333333] mb-1">Label</label>
            ...
            <div class="pt-2">
                <label class="flex items-center text-xs text-[#555555] cursor-pointer">
                    <input type="checkbox" name="is_default" value="1" {{ $address->is_default ? 'checked' : '' }} class="mr-2 rounded text-[#1C1C1C] focus:ring-[#D8C3A5]">
                    <span>Set as primary default address</span>
                </label>
            </div>
            <div class="pt-4 flex items-center space-x-4">
                <button type="submit" class="btn-primary text-xs">Update Address</button>
